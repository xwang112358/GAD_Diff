...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self, data_loaders, compute_emd, metrics_list, augment=True):
        super().__init__()
        print(augment)
        if augment:
            self.train_graphs = self.loader_to_nx(data_loaders)
            self.val_graphs = self.loader_to_nx(data_loaders)
            self.test_graphs = self.loader_to_nx(data_loaders)
        else:
