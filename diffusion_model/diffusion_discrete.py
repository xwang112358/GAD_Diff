...
from diffusion_model.models.transformer_model import GraphTransformer
from diffusion_model.diffusion.noise_schedule import DiscreteUniformTransition, PredefinedNoiseScheduleDiscrete,\
    MarginalUniformTransition
from diffusion_model.diffusion import diffusion_utils
from diffusion_model.metrics.train_metrics import TrainLossDiscrete
from diffusion_model.metrics.abstract_metrics import SumExceptBatchMetric, SumExceptBatchKL, NLL
...
class DiscreteDenoisingDiffusion(pl.LightningModule):   # replace domain_features 
    def __init__(self, cfg, input_dims, output_dims, nodes_dist, node_types, edge_types, extra_features, domain_features, data_loaders, sampling_metrics, augment = False):
        ...
        if augment:
            ...
            self.val_loader = data_loaders 
            self.test_loader = data_loaders
        else:
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # Generate and visualize graphs, and further evaluate the mmd on validation data
        ...
        #     self.sampling_metrics.forward(samples, test=False, local_rank=self.local_rank)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        while samples_left_to_generate > 0:
            ...
            bs = 4 * self.cfg.train.batch_size
            to_generate = min(samples_left_to_generate, bs)
            to_save = min(samples_left_to_save, bs)
            chains_save = min(chains_left_to_save, bs)
        ...
        self.sampling_metrics(samples, test=True, local_rank=self.local_rank)
