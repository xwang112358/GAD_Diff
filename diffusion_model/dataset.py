import torch
from diffusion_model.distributions import DistributionNodes
from diffusion_model.utils import to_dense
...
#     node_types = torch.tensor([1]) # distribution of node types: normal and anomalous
...
def init_dataset(dataset_name, dataloader):
    print(f'augmenting {dataset_name} ...')
    n_nodes = node_counts(1000, dataloader)
    node_types = torch.tensor([1])  # extract metadata from the dataset
    edge_types = edge_counts(dataloader)
    num_classes = len(node_types)
    max_n_nodes = len(n_nodes) - 1
    nodes_dist = DistributionNodes(n_nodes)
...
def node_counts(max_nodes_possible, data_loader):
